...
def main(config_path: str, **kwargs):
    ...
    if "format_date" in dc["script_config"]:
        data.index = pd.to_datetime(
            data.index, format=dc["script_config"]["format_date"]
        )
        data.index = pd.Series(data.index).dt.tz_localize(
            dc["script_config"]["timezone"]
        )
    else:  # formatDate == 2 IBKR for epoch seconds
        data.index = pd.to_datetime(data.index, unit="s")
        data.index = pd.Series(data.index).dt.tz_localize("UTC")
        data.index = pd.Series(data.index).dt.tz_convert(
            dc["script_config"]["timezone"]
        )
